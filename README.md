...
### 6. 实体消歧合并
- 解析阶段自动识别同义实体（如"北大"和"北京大学"、"中科院"和"中国科学院"）并合并为一个节点
- 原始名称保留为别名，搜索任一名称都能定位到合并后的实体
- 同义识别策略：内置别名词典 + 机构后缀剥离 + 前缀包含 + 中文简称子序列规则
- 合并时自动迁移关系、累加计数，关系两端的旧名称同步归一
- 启动时自动清理存量图谱中的重复实体
- 支持手动添加别名（`POST /api/graph/aliases`）
...
│   ├── graph/
│   │   ├── builder.py      # 图谱构建
│   │   ├── entity_resolver.py  # 实体消歧（同义识别与归一）
│   │   ├── query.py        # 图谱查询
...
### 3. 实体消歧合并
- 内置别名词典（高校/科研机构/企业常见简称）
- 中文简称子序列规则：短名首字与全称首字相同且字符按序出现（北大⊂北京大学）
- 机构后缀剥离与前缀包含匹配（腾讯↔腾讯公司、阿里巴巴↔阿里巴巴集团）
- 同类型约束防止误合并（"北京"地点 与 "北京大学"机构 不会合并）
- 别名索引持久化（aliases.json），读写路径双向生效
...
### 实体消歧
- `GET /api/graph/aliases` - 获取所有标准实体及其别名列表
- `POST /api/graph/aliases` - 手动添加别名（`{"alias": "北大", "canonical": "北京大学"}`，若别名实体已存在则触发合并）
