...
- 节点可拖拽、缩放
- 按实体类型筛选
- 实体搜索功能（支持全称、简称、英文缩写等别名，如搜索"北大"可定位"北京大学"）
- 节点详情查看（展示合并的全部别名）
...
│   │   ├── tokenizer.py    # 中文分词
│   │   ├── ner.py          # 命名实体识别
│   │   ├── entity_resolver.py  # 实体消歧（同义识别/简称缩写归并）
...
2. 系统自动进行：
   - 中文分词
   - 命名实体识别
   - 实体消歧合并（同义实体自动归并，如"北大"合并到"北京大学"）
   - 关系抽取
   - 三元组生成
3. 解析结果中可查看消歧合并详情（规范名 ⇐ 原始名称）
...
- 三元组: JSON文件（按文档分文件）
- 图谱: JSON分片（按实体类型）
- 别名索引: `data/graph/aliases.json`（同义名称 -> 规范实体，按类型隔离）
- 会话: JSON文件（按会话ID）
...
### 2. 实体消歧与关系抽取
- 内置同义词典：常见高校、科研机构、企业、地名、科技术语的全称/简称/英文名
- 文本简称声明识别：`北京大学（以下简称北大）`、`中科院（简称中国科学院）`、`麻省理工学院（MIT）`
- 类型安全的并查集聚类：仅同类型实体合并，避免跨类型误合
- 同义实体合并为一个节点，原始名称全部保留为别名（aliases）
- 写入/查询透明重定向：任一别名均可定位到规范实体及其全部关系
- 存量数据启动迁移：旧图谱中的分散节点自动合并，关系随之改写并去自环
- 基于模式匹配的关系抽取
- 置信度评估
